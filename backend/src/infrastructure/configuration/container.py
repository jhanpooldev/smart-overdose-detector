...
Selecciona automáticamente el repositorio según STORAGE_BACKEND en el .env:
  - STORAGE_BACKEND=memory   → Repositorios en memoria (sin BD, datos vacíos)
  - STORAGE_BACKEND=postgres → Repositorios PostgreSQL / TimescaleDB reales
"""
import logging
...
if settings.STORAGE_BACKEND == "postgres":
    ...
    signal_repository  = PostgresSignalRepository(settings.DATABASE_URL)
    risk_repository    = PostgresRiskRepository(settings.DATABASE_URL)
    logger.info("🗄️  Repositorio: PostgreSQL — %s", settings.DATABASE_URL.split("@")[-1])
    ...
    from src.infrastructure.adapters.output.persistence.in_memory_risk_repository import InMemoryRiskRepository

    signal_repository  = InMemorySignalRepository()
    user_repository    = InMemoryUserRepository()
    risk_repository    = InMemoryRiskRepository()
    logger.info("🗄️  Repositorio: Memoria RAM (sin BD persistente)")

# Repositorios auxiliares (siempre en memoria por ahora)
from src.infrastructure.adapters.output.persistence.in_memory_contact_repository import InMemoryContactRepository
from src.infrastructure.adapters.output.persistence.in_memory_settings_repository import InMemorySettingsRepository

contact_repository  = InMemoryContactRepository()
settings_repository = InMemorySettingsRepository()

# ──────────────────────────────────────────────────────────────
# 2. Adaptadores externos y servicios de aplicación
# ──────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────
# 3. Servicios de dominio y casos de uso
# ──────────────────────────────────────────────────────────────
